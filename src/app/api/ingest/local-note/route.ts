...
import { ingestLocalNote, parseLocalNotePayload } from "@/ingestion/local-note";
...
async function persistIfRequested(
  payload: unknown,
  ingestion: ReturnType<typeof ingestLocalNote>
) {
...
export async function POST(request: Request) {
  try {
    ...
    const input = parseLocalNotePayload(payload, {
      organizationId: session.organizationId,
      createdBy: session.userId
    });
    const ingestion = ingestLocalNote(input);
